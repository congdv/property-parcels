    ...
    <div style={{ position: 'fixed', left: popup.x + 8, top: popup.y + 8, zIndex: 1000 }}>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: '#333' }}>
          {popup.props?.formatted_value ? `$${popup.props.formatted_value}` : ''}
        </div>
        <div style={{ fontSize: 13, color: '#666', marginTop: 6 }}>
          <strong style={{ fontWeight: 600 }}>County:</strong>{' '}
          {formatCounty(popup.props?.county) || 'Unknown'}
        </div>
        <div style={{ fontSize: 13, color: '#333', marginTop: 4 }}>
          <strong style={{ fontWeight: 600 }}>Area:</strong>{' '}
          {popup.props?.sqft != null ? `${Number(popup.props.sqft).toLocaleString()} sqft` : '-'}
        </div>
